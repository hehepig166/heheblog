...
def get_followlist(userid):
    cdb.fetchall()
    # cdb.execute("SELECT followwho FROM follows WHERE whofollow = ? ORDER BY followwho", [userid])
    cdb.execute("SELECT userid, nickname FROM users WHERE userid IN (SELECT followwho FROM follows WHERE whofollow = ?)", [userid])
    return cdb.fetchall()
